package com.theforum.constants;

/**
 * @author DEEPANKAR
 * @since 31-01-2016.
 */
public class Messages {

    /**
     * error messages
     */

    public static final String NO_NET_CONNECTION = "Check your Internet Connection";
    public static final String SERVER_ERROR = "Cannot Establish a proper connection to Server";
    public static final String TOPIC_DELETE = "24 hrs of this topic has been completed";
    public static final String NO_LOCAL_TOPIC = "Your local feed has no topic or your location cannot be traced";

}
